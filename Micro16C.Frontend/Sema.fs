...
and Statement =
    | WhileStatement of Expression: Expression * Statement: Statement
    | DoWhileStatement of Statement: Statement * Expression: Expression
    | ForStatement of For
    | IfStatement of Expression * Statement * Statement option
    | BreakStatement of Lazy<Statement option>
    | ContinueStatement of Lazy<Statement option>
    | CompoundStatement of CompoundItem list
    | ExpressionStatement of Expression option
    | LabelStatement of Statement
    | GotoStatement of Lazy<Statement option>
...
type Context =
    { Scopes: Map<string, Declaration> list
      Loops: Lazy<Statement option> list
      SourceObject: SourceObject
      Labels: Map<string, Statement>
      Gotos: (Token * Lazy<Statement option>) list }
...
and visitAssignmentExpression (context: Context) (expression: Parse.AssignmentExpression) : Result<Expression, string> =
    ...
    | list ->
        ...
        let lastExpr, list =
            list
            |> List.fold
                (fun (lastExpr, result) (token, expr) -> (expr, (lastExpr, token) :: result))
...
and visitConditionalExpression
    (context: Context)
    (expression: Parse.ConditionalExpression)
...
and visitInclusiveOrExpression
    (context: Context)
    (expression: Parse.InclusiveOrExpression)
    : Result<Expression, string> =
...
and visitExclusiveOrExpression
    (context: Context)
    (expression: Parse.ExclusiveOrExpression)
    : Result<Expression, string> =
        ...
        |> List.fold
            (fun lhs (token, rhs) ->
                ...
                | Ok _, Ok _ ->
                    ...
                    $"%s{opName} only supported on integer types"
                    |> context.SourceObject.emitError (ErrorTypeToken token)
                    |> Error)
            expression
    ...
    | list ->
        ...
        |> List.fold
            (fun lhs (token, rhs) ->
                ...
                | (Ok (Type (PointerType _ as resultType) as lhs),
                   Ok (Type (PointerType _) as rhs),
                ...
                | Ok lhs, Ok rhs, _ ->
                    ...
                    $"%s{opName} not allowed on operators of type %s{lhs |> Expression.getType |> typeToString} and %s{rhs |> Expression.getType |> typeToString}"
                    |> context.SourceObject.emitError (ErrorTypeToken token)
                    |> Error)
            expression

and visitMultiplicativeExpression
    (context: Context)
        ...
        |> List.fold
            (fun lhs (token, rhs) ->
                ...
                | Ok _, Ok _ ->
                    ...
                    $"%s{opName} only supported on %s{typeName} types"
                    |> context.SourceObject.emitError (ErrorTypeToken token)
                    |> Error)
            expression
...
let rec visitStatement (context: Context) (statement: Parse.Statement) =
    ...
    | Parse.WhileStatement (expression, statement) ->
        ...
        let rec whileLoop =
            (fun _ ->
                ...
                let semaStatement = visitStatement context statement |> fst

                comb2 (fun x y -> WhileStatement(x, y)) condition semaStatement)
                ()
        ...
        let rec doWhileLoop =
            (fun _ ->
                let context =
                    { context with
                          Loops =
                              lazy (doWhileLoop |> resultToOption)
                              :: context.Loops }
                ...
                comb2 (fun x y -> DoWhileStatement(y, x)) condition statement)
                ()
    ...
    | Parse.ForStatement (_, condition, iteration, body) ->
        ...
        let rec forLoop =
            (fun _ ->
                let context =
                    { context with
                          Loops = lazy (forLoop |> resultToOption) :: context.Loops }

                let statement, _ = visitStatement context body
    ...
    | Parse.GotoStatement name ->
        let goto =
            lazy
                (context.Labels
                 |> Map.tryFind (Token.identifier name))

        (goto |> GotoStatement |> Ok,
         { context with
               Gotos = (name, goto) :: context.Gotos })
...
and visitDeclaration (context: Context) (declaration: Parse.Declaration) =
    ...
    |> List.fold
        (fun (list, context) (declarator, maybeAssignment) ->
            ...
            let element, context =
                ...
                | Some _ ->
                    ($"Redeclaration of variable '%s{identifier}'"
                ...
                | None ->
                    ...
                    match
                        maybeAssignment
                        |> Option.map (visitAssignmentExpression context)
                        with
            ...
            (prependResult element list, context))
        ([] |> Ok, context)
...
and visitCompoundStatement (context: Context) (translationUnit: Parse.CompoundItem list) =
    List.fold
        (fun (list, context) x ->
            ...
            (prependResult item list, context))
        ([] |> Ok, context)
        translationUnit
    |> fun (x, y) -> (x |> Result.map List.rev, y)
...
let analyse (sourceObject, translationUnit) =
    ...
    let error =
        context.Gotos
        |> List.fold
            (fun result (token, label) ->
                if label.Force() |> Option.isSome then
                    result
                else
                    let error =
                        $"Unresolved label '%s{Token.identifier token}'"
                        |> context.SourceObject.emitError (ErrorTypeToken token)
                        |> Error

                    comb2 (fun _ _ -> ()) result error)
            (() |> Ok)
